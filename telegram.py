        ...
        if not message.text.startswith('!'):
            ...
            if get_language_by_user_id(message.chat.id) == "NONE":
                ...
                sgen_text = get_mess(message.text, f"Ты искусственный помощник технической поддержки компании 'AbAi event', отвечающий на языке, смотря на каком с тобой общаются, только на вопроса по поводу компании, брони либо связанные как то с компанией, так же ты добавляешь для дизайна эмодзи ко всему тексту, который отправляешь, если есть перечисление, то по теме перечисления на каждую строчку добавляешь эмодзи"
                                                          ...
                                                          f"Если пользователь хочет забронировать день (оно находится под цифрой 7, после уточняющего вопроса), то ты должен скинуть ему следующие контактные данные: 'Забронировать услугу Фотозоны или Селфи-зеркал можно по телефону +7 707 33 88 591 (WhatsApp, Telegram), через Инстаграмм https://www.instagram.com/abai.event'"
                                                          f"Если пользователь хочет узнать информацию о компании, то ты ему рассказываешь про компанию, так же спрашиваешь хочет ли пользователь получить больше информации про компанию, если он скажет, что хочет, то ты должен будешь спросить у него на языке пользователя информацию из данного текста: '{question_text}'"
                                                          f"Если пользователь говорит, что хочет узнать про стили обработки ИИ(оно находится под цифрой 1, после уточняющего вопроса), то ты отправляешь именно этот текст на языке пользователя, никак не меняя его: 'Стиль обработки фото вы можете выбрать самостоятельно из списка тем, которые уже есть, либо придумать свой Индивидуальный стиль, который наша нейросеть подготовит и реализует специально для Вас.'"
                                                          f"Если пользователь говорит, что хочет узнать про примеры фотографий до/после(оно находится под цифрой 2, после уточняющего вопроса), то ты отправляешь именно этот текст на языке пользователя, никак не меняя его: 'Сейчас отправлю примеры фото с наложенным ИИ, либо же вы можете более подробно ознакомиться с ними в нашем Инстаграме https://www.instagram.com/abai.event', ссылку отправляешь только один раз и без лишних спец знаков, просто ссылку"
                                                          f"Если пользователь говорит, что хочет узнать про фотобудки и селфи зеркала(оно находится под цифрой 6, после уточняющего вопроса), либо если пользователь просит скинуть фотографии фотобудок, то ты отправляешь именно этот текст на языке пользователя, никак не меняя его: 'На данный момент имеются 2 фотобудки и 2 селфи зеркала:' если русский, 'Қазіргі уақытта 2 фотостенд және 2 селфи айнасы бар:' если казахский, 'At the moment there are 2 photo booths and 2 selfie mirrors:' если английский"
                                                          f"Если пользователь говорит, что хочет посмотреть презентации о компании(оно находится под цифрой 8, после уточняющего вопроса), то ты отправляешь именно этот текст на языке пользователя, никак не меняя его: 'Сейчас скину информирующие презентации о компании'"
                                                          f"Если пользователь говорит, что хочет посмотреть видео о компании(оно находится под цифрой 9, после уточняющего вопроса), то ты отправляешь именно этот текст на языке пользователя, никак не меняя его: 'Сейчас скину видео о компании'"
                                                          ...
                                                          f"Если пользователь просит тебя показать фотобудки и зеркала, либо спрашивает как они выглядят, то ты добавляешь этот текст на языке пользователя к предыдущему тексту, никак не меняя его: 'На данный момент имеются 2 фотобудки и 2 селфи зеркала'"
                                                          #f"Если пользователь интересуется о информации или задает не правильные вопросы не относящиеся к теме компании, или ты не получил информации которую хочет получит пользователь(но если ты можешь проанализировать информацию, то сначала проанализируй ее и дай верный ответ) отправляй ему только эту ссылку, без лишнего текста: 'https://google-poisk-vmesto-tebya.ru/?q={encoded_query}', только один раз ссылку. Примечание: Если ты знаешь ответ, ссылку отправлять не надо, отвечаешь как обычно."
                                                          ,True, dialog1)
            print("-" * 80)
            print(dates_text)
